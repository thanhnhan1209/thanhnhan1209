...
public class ThongKeForm extends javax.swing.JInternalFrame {
    ...
    public ThongKeForm() {
        ...
        changeTextFind();
        //
        loadDataToTableAcc((ArrayList<Account>) AccountDAO.getInstance().selectAll());
        ...
        //
        loadDataToTableThongKeProduct((ArrayList<ThongKeProduct>) ThongKeDAO.getInstance().getThongKe());
    }
    ...
    private void loadDataToTable() {
        try {
            ArrayList<PhieuNhap> allPhieu = (ArrayList<PhieuNhap>) PhieuNhapDAO.getInstance().selectAll();
    ...
    public ArrayList<Phieu> searchTatCa(String text) {
        ...
        ArrayList<PhieuNhap> armt = PhieuNhapDAO.getInstance().selectAllP();
        for (var phieu : armt) {
    ...
    public ArrayList<Phieu> searchPhieuNhap(String text) {
        ArrayList<Phieu> result = new ArrayList<>();
        ArrayList<PhieuNhap> armt = (ArrayList<PhieuNhap>) PhieuNhapDAO.getInstance().selectAll();
        for (var phieu : armt) {
            if (phieu.getMaPhieu().toLowerCase().contains(text.toLowerCase())
                    || phieu.getNguoiTao().toLowerCase().contains(text.toLowerCase())) {
    ...
    public ArrayList<Phieu> searchPhieuXuat(String text) {
        ArrayList<Phieu> result = new ArrayList<>();
        ArrayList<PhieuXuat> armt = (ArrayList<PhieuXuat>) PhieuXuatDAO.getInstance().selectAll();
        for (var phieu : armt) {
            if (phieu.getMaPhieu().toLowerCase().contains(text.toLowerCase())
                    || phieu.getNguoiTao().toLowerCase().contains(text.toLowerCase())) {
                result.add(phieu);
    ...
    private void initComponents() {
        ...
        tabThongKe.setPreferredSize(new java.awt.Dimension(1180, 770));
        ...
        tabThongKe.addTab("Sản phẩm", jPanel12);
        ...
        tabThongKe.addTab("", jPanel14);

        getContentPane().add(tabThongKe, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 130, -1, 620));
    ...
    private void btnXemBieuDoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXemBieuDoActionPerformed
        ...
        List<ThongKeDoanhThuPhanLoai> data = ThongKeDAO.getInstance().getThongKeDoanhThuTheoPhanLoai();
        new ThongKeBieuDo(null, data).setVisible(true);
        hienThiBieuDoDoanhThuTheoNgay();
        hienThiBieuDoSoLuongNhapXuat();
...
public PhieuNhap findPhieu() {
    int i_row = tblPhieuNhap.getSelectedRow();
    if (i_row == -1) return null;

    String maPhieu = tblModel.getValueAt(i_row, 0).toString();
    ArrayList<PhieuNhap> dsPhieu = PhieuNhapDAO.getInstance().selectAllP();
    
    for (PhieuNhap phieu : dsPhieu) {
    ...
    public void filterThongKeSanPham() throws ParseException {
        ...
        if (jDateChooserFromPr.getDate() != null || jDateChooserToPr.getDate() != null) {
            ...
            thongKe = (ArrayList<ThongKeProduct>) ThongKeDAO.getInstance().getThongKe(from, to);

        } else {
            thongKe = (ArrayList<ThongKeProduct>) ThongKeDAO.getInstance().getThongKe();
        }
...
private void hienThiBieuDoDoanhThu() {
    try {
        // Lấy dữ liệu từ DAO
        List<ThongKeDoanhThuPhanLoai> danhSach = ThongKeDAO.getInstance().getThongKeDoanhThuTheoPhanLoai();
        ...
        for (ThongKeDoanhThuPhanLoai item : danhSach) {
            dataset.addValue(item.getTongTien(), "Doanh thu", item.getPhanLoai());
        }

        // Tạo biểu đồ
        JFreeChart chart = ChartFactory.createBarChart(
            "Biểu đồ doanh thu theo phân loại", // tiêu đề
            "Phân loại",                       // trục X
        ...
        JOptionPane.showMessageDialog(this, "Đã xảy ra lỗi khi hiển thị biểu đồ.", "Lỗi", JOptionPane.ERROR_MESSAGE);
        e.printStackTrace();
    }}
private void btnXemBieuDoActionPerformed() {
    List<ThongKeDoanhThuPhanLoai> data = ThongKeDAO.thongKeDoanhThuTheoPhanLoai();
    ...
    ThongKeBieuDo dialog = new ThongKeBieuDo(parentWindow, data);
    dialog.setVisible(true);
}private void hienThiBieuDoDoanhThuTheoNgay() {
    ThongKeDAO thongKeDAO = new ThongKeDAO();
    List<Object[]> duLieu = thongKeDAO.getDoanhThuTheoThoiGian();

    DefaultCategoryDataset dataset = new DefaultCategoryDataset();
    for (Object[] row : duLieu) {
        String ngay = row[0].toString();
        double doanhThu = (double) row[1];
    ...
    JFrame frame = new JFrame("Biểu đồ doanh thu theo thời gian");
    frame.setContentPane(chartPanel);
    frame.pack();
    frame.setLocationRelativeTo(null);
    frame.setVisible(true);
}
private void hienThiBieuDoSoLuongNhapXuat() {
    ThongKeDAO dao = new ThongKeDAO();
    List<Object[]> duLieu = dao.getSoLuongNhapXuatTheoSanPham();
    ...
    for (Object[] row : duLieu) {
        ...
        Number slNhap = ((Number) row[2] != null) ? (Number) row[2] : 0;
        Number slXuat = ((Number) row[3] != null) ? (Number) row[3] : 0;

        dataset.addValue(slNhap, "Số lượng Nhập", tenSP);
        dataset.addValue(slXuat, "Số lượng Xuất", tenSP);
    }

    JFreeChart barChart = ChartFactory.createBarChart(
            "So sánh số lượng Nhập - Xuất theo sản phẩm",
            "Sản phẩm",
            "Số lượng",
            dataset,
            PlotOrientation.VERTICAL,
    ...
    JPanel panelBieuDo = new JPanel(); // hoặc thay bằng panel bạn đã tạo sẵn trong GUI
    ...
    // Nếu bạn đã có panel riêng trong form, ví dụ: pnlBieuDoSoLuong
